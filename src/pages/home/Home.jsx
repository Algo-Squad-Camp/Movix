...
function Home() {
  ...
  const languagen = details && details?.language;
  const title76 = `Because Your native language is '${languagen}'`
  useEffect(() => {
    const countryyy = languagelist.map(item => {
      if (languagen === item.english_name) {
        setNativeLanguage(item.iso_639_1);
      }
    })
  }, [nativeLanguage]);
    ...
    <>
      ...
      <div className='homebanner'>
        <div className="corousal">
          <Trending />
          <TrendingCast />
          {details?.language && <NativeLanguage title={title76} language={nativeLanguage} />}
          <Streaming />
          <Language />
